...
def connected_to_fixed(state, node, fixed_positions, nodes_to_ignore=None):
...
def add_pos_to_state(state):
    ...
    node_size = 150
    env_nodes = ["env/observations", "env/actions", "env/render"]
    G = nx.Graph()
    fixed_positions = {}
    ...
    for node, params in state["nodes"].items():
        ...
        elif params["config"]["name"] in env_nodes:
            x_pos = node_size * (2 + max((len(state["nodes"].items()) - 3) // 2, 2))
            if params["config"]["name"] == env_nodes[1]:
                pos = [0, 0]
            elif params["config"]["name"] == env_nodes[0]:
                pos = [x_pos, 0]
            else:
                pos = [x_pos, node_size]
            fixed_positions[node] = pos
        elif not connected_to_fixed(state, node, fixed_positions):
